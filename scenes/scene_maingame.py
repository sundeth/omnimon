...
class SceneMainGame:
    ...
    def update_static_surface(self) -> None:
        ...
        if not self.cached_static_surface or menu_index_changed or clock_changed:
            ...
            self.cached_static_surface = pygame.Surface((runtime_globals.SCREEN_WIDTH, runtime_globals.SCREEN_HEIGHT))
    ...
    def draw_pet(self, surface: pygame.Surface, pet, index: int, selected_pets: set, show_hearts: bool) -> None:
        ...
        if show_hearts:
            self.draw_hearts(surface, pet.x + (runtime_globals.PET_WIDTH // 4), pet.y + runtime_globals.PET_HEIGHT, pet.hunger)
            self.draw_hearts(surface, pet.x + (runtime_globals.PET_WIDTH // 4), pet.y + runtime_globals.PET_HEIGHT + (6 * runtime_globals.UI_SCALE), pet.strength)
