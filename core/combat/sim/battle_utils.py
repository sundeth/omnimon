...
def load_dm20_tag_patterns():
    ...
    try:
        json_path = os.path.join(os.path.dirname(__file__), '..', 'protocols', 'battle', 'attack_patterns', 'DM20.json')
...
def get_attack_pattern(level, mini_game, protocol="DMX"):
    if protocol == "DMX":
        ...
        if DMX_PATTERN_TABLE is None:
            pattern_path = os.path.join(os.path.dirname(__file__), "..", "protocols", "battle", "attack_patterns", "DMX.json")
...
def get_dm20_attack_pattern(tag_meter, taps):
    ...
    if DM20_PATTERN_TABLE is None:
        ...
        pattern_path = os.path.join(os.path.dirname(__file__), "..", "protocols", "battle", "attack_patterns", "DM20.json")
        with open_json(pattern_path) as f:
